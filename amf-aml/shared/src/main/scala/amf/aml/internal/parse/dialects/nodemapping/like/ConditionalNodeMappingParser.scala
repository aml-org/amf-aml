        ...
        entry => {
          entry.value.tagType match {
            case YType.Str =>
              ...
              conditionalNodeMapping.set(field, AmfScalar(value, Annotations(entry.value)), Annotations(entry))
            case _ => // Already tested in closed shape?
//            ctx.eh.violation(
//              DialectError,
//              conditionalNodeMapping.id,
//              s"${key.capitalize} field in a conditional node mappings must be a declared node mapping reference",
//              entry.value.location
//            )
          }
        }
