    ...
    map.key(
        ...
        entry => {
          ...
          val parsed = references.map { node: YNode =>
            ...
            reference match {
              case Some(_) =>
                (reference, NodeMappingLikeParser.resolveNodeMappingLink(map, node, adopt)) // we don't need to adopt, it is a link
          ...
          nodeMapping.withExtends(resolved).extend.zipWithIndex.foreach {
