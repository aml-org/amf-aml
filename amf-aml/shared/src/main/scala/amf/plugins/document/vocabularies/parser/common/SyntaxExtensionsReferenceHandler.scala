...
class SyntaxExtensionsReferenceHandler(registry: DialectsRegistry, eh: AMFErrorHandler) extends ReferenceHandler {
  ...
  private def collect(parsedDoc: ParsedDocument)(implicit errorHandler: AMFErrorHandler) = {
    parsedDoc match {
