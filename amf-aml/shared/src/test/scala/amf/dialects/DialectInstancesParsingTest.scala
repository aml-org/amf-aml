...
trait DialectInstancesParsingTest extends DialectTests {
  ...
  test("Clone instance from dialect") {
    ...
    val context1 =
      new CompilerContextBuilder(s"file://$basePath/dialect31.yaml", platform, DefaultParserErrorHandler.withRun())
        .withBaseEnvironment(env)
        .build()
    val context2 =
      new CompilerContextBuilder(s"file://$basePath/example31.yaml", platform, DefaultParserErrorHandler.withRun())
        .withBaseEnvironment(env)
        .build()
    for {
