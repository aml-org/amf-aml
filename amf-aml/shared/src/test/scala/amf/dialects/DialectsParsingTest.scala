...
trait DialectsParsingTest extends DialectTests {
  ...
  multiSourceTest("generate 8 test", "example8.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example8.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 9 test", "example9.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example9.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 10 test", "example10.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example10.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 11 test", "example11.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example11.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 12 test", "example12.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example12.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 13 test", "example13.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example13.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 14 test", "example14.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example14.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 15 test", "example15.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example15.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 16 test", "example16.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example16.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 17 test", "example17.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example17.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 18 test", "example18.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example18.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 19 test", "example19.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example19.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 20 test", "example20.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example20.yaml", AmfJsonHint, target = Aml))
  }
  ...
  multiSourceTest("generate 22 test", "example22.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example22.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 23a test", "example23a.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example23a.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate 23b test", "example23b.%s") { config =>
    init().flatMap(_ => cycle(config.source, "example23b.yaml", AmfJsonHint, target = Aml))
  }
  ...
  multiSourceTest("no documents on dialect (json -> raml)", "no-documents.%s") { config =>
    init().flatMap(_ => cycle(config.source, "no-documents.yaml", AmfJsonHint, target = Aml))
  }

  multiSourceTest("generate mappings_lib test", "mappings_lib.%s") { config =>
    init().flatMap(_ => cycle(config.source, "mappings_lib.yaml", AmfJsonHint, target = Aml))
  }
  ...
  multiSourceTest("generate 19 test - with key property", "keyproperty/example19-keyproperty.%s") { config =>
    init().flatMap(_ => cycle(config.source, "keyproperty/example19-keyproperty.yaml", AmfJsonHint, target = Aml))
  }
  ...
  multiSourceTest("generate 19 test - with reference style", "referencestyle/example19-referencestyle.%s") { config =>
    init().flatMap(_ => cycle(config.source, "referencestyle/example19-referencestyle.yaml", AmfJsonHint, target = Aml))
  }
