...
//  }
//
//  private def withFinder[T](path: String, factoryFunc: AMLConfiguration => Seq[SemanticExtensionFinder])(
//      block: SemanticExtensionFinder => T): Future[T] = {
//    for {
//      config <- AMLConfiguration.predefined().withDialect(path)
//    } yield {
//      val finder = factoryFunc(config)
//      block(finder)
//    }
//  }
//
