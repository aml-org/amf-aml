...
import amf.plugins.validation.emitters.{JSLibraryEmitter, ValidationJSONLDEmitter}
import amf.plugins.validation.model.{ParsedValidationProfile, ValidationDialectText}
...
object AMLValidationPlugin extends ParserSideValidationPlugin with PlatformSecrets {

  override val ID = "AML Validation"

  override def init(): Future[AMFPlugin] = {
    // Registering ourselves as the runtime validator
    RuntimeValidator.register(AMLValidationPlugin)
