...
import amf.core.metamodel.domain.{DomainElementModel, LinkableElementModel, ModelDoc, ModelVocabularies, ExternalModelVocabularies}
...
object NodeMappingModel extends DomainElementModel with LinkableElementModel with MergeableMappingModel with NodeMappableModel {
  ...
  val NodeTypeMapping   = Field(Iri, Namespace.Shacl + "targetClass",
    ModelDoc(ExternalModelVocabularies.Shacl, "target class", "Target class whose instances will need to match the constraint described for the node"))
  ...
  val IdTemplate        = Field(Str, Namespace.ApiContract + "uriTemplate",
    ModelDoc(ModelVocabularies.ApiContract, "uriTemplate", "URI template that will be used to generate the URI of the parsed nodeds in the graph"))
