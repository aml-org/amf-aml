...
object ExternalModel extends DomainElementModel {
  ...
  val DisplayName = Field(Str, Namespace.Core + "displayName", ModelDoc(ModelVocabularies.Core, "displayName", "Display name for an external model"))
  val Base = Field(Str, Namespace.Meta + "base", ModelDoc(ModelVocabularies.Meta, "base", "Base URI for the external model"))
