...
case class DialectDeclaresParser(into:DocumentMapping)(override implicit val ctx:DialectContext) extends DialectEntryParser {

  def parse(map:YMap):Unit = map.key("declares", parse)

  override def parse(entry:YMapEntry):Unit = {
    val declaresMap = entry.value.as[YMap]
    val declarations: Seq[PublicNodeMapping] = declaresMap.entries.flatMap {
      declarationEntry =>
        ...
        ctx.declarations.findNodeMapping(declarationId, SearchScope.All).map { nodeMapping =>
          declarationMapping.set(PublicNodeMappingModel.MappedNode,
            AmfScalar(nodeMapping.id, Annotations(declarationEntry.value)), Annotations(declarationEntry.value))
        }
    }
